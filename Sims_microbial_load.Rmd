...
design <- as.formula("~ cohort + timepoint + (1 + timepoint | patientID)")

fit_ANI = glmFit(se_98_ANI, design, 10L)
top_hits(fit_ANI)
plot_manhattan(fit_ANI)

fit_AB = abundanceFit(se = se_98_abundance, design = design, nthreads = 10L, transform = 'CLR')
top_hits(fit_AB)
plot_manhattan(fit_AB)
